...
Definitions for a graph abstract data type, including a node
signature, a graph signature, and a functor for building graphs from a
node module.
 *)
...
module type NODE =
  sig
    ...
    val compare : node -> node -> Order.ordering
...
module Graph (NA : NODE) : (GRAPH with module N = NA) =
  struct
    ...
    (* We'll represent a graph as an edge dictionary mapping nodes to
       the set of their neighbor nodes.  Every node in the graph must
       appear as a key in the dictionary.  *)
    ...
    let nodes g =
      EdgeDict.fold (fun r k _ -> k :: r) [] g.edges ;;
    ...
    let add_edge g src dst =
      let new_neighbors =
        ...
        | None -> NeighborSet.insert NeighborSet.empty dst
        | Some s -> NeighborSet.insert s dst
      in
    ...
    let outgoing_edges g src : (node * node) list option =
      ...
      | None -> None
      | Some s ->
          Some (NeighborSet.fold
                  (fun r dst -> (src, dst) :: r) [] s) ;;
