...
Definitions for node scores maps, which are used as part of the
page-rank algorithm.
 *)
...
module type NODE_SCORE =
  sig
    ...
    (* Scale the scores so that they add to 1.  If they originally add
       to zero, leaves them unchanged *)
